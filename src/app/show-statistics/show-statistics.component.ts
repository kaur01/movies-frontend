...
import notebook from '../../assets/graph.js';
...
export class ShowStatisticsComponent implements AfterContentInit {
...
// only respond once per keydown
  ...
  constructor(elementRef: ElementRef, statisticsService: StatisticsService, renderer: Renderer2) {
  ...
  public displayOptions() {
    if (this.selected === 'actor') {
      this.options = [];
      for (let i = 0; i < 10; i++) {
        this.options.push('actor' + i);
      }
    } else if (this.selected === 'movie') {
      this.options = [];
      for (let i = 0; i < 10; i++) {
        this.options.push('Good Fellas');
      }
    } else if (this.selected === 'director') {
      this.options = [];
      for (let i = 0; i < 10; i++) {
        this.options.push('Christopher Nolan');
      }
    }
  }

  public async fetchStats(selection: string) {
    const stats = await this.statisticsService.getStatsFor(selection);
    console.log(stats);
  }

  ngAfterContentInit() {
    const rect = this.directedGraph.nativeElement.getBoundingClientRect();
    console.log(rect.width, rect.height);

    this.width = rect.width;

    this.svg = d3.select('#directedGraph')
